...
const provider = (<Provider store={store}><Router history={history}><Entry /></Router></Provider>);
...
describe('render into document with store and use enzyme', () => {
	it('try to render into document by enzyme', () => {
		...
		// const wrapper = mount(provider);
		wrapper.update();
		console.log(wrapper.find('.menutype'));
    wrapper.find({id:'Record'}).first().simulate('click');
		expect(wrapper.find('div.entry-body')).to.have.length(1);
	});
});
